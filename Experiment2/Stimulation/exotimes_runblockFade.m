...
function [r,nblock,times] = exotimes_runblockFade(p,c,d,r,t,nblock,times)
...
Performance = [];
clear thisTrials; 
thisTrials =  exotimes_stimuliFade(); 
durations = exotimes_duracionesFade();
grados = exotimes_gradosFade();
...
for ntrial = 1:length_trials
    ...
    nima = thisTrials(ntrial); % stim ID (1:120)
    ...
    ncond = exotimes_condicionesFade(nima); % de 1 a 3
    ...
    % 1) Display stimulus for stimulus duration
    nhz = 1;

    duracion = nduracion + p.fadetime;
    iniciofade = duracion - p.fadetime;
    tic
    while nhz <= duracion 
    ...
    % 2) Display blank screen during ITI (+ jitter) duration
    nhz = 1;
